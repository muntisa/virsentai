...
Reads virus prediction data from a TSV file and inserts it
...
Requirements:
    - SQLite database at db/virsentai.sqlite3
    - TSV file with columns: created_at, PClass_1, accession, organism, host, source
"""
import os
import csv
import logging
...
def add_predictions_to_db(tsv_file_path, sqlite_db_path, modelid):
    ...
    try:
        ...
        for index, row in df.iterrows():
            prediction_data = (
                row[column_mapping["Registration_Date"]],
                row["PClass_1"],
                modelid,
                row[column_mapping["Virus_ID"]],
                row[column_mapping["Virus_Name"]],
                row[column_mapping["Host"]],
                row[column_mapping["Database"]],
